...
chi_steps = np.arange(0.01, 2, 0.2)
for chi_ini in chi_steps:

        sol_object = optimize.root(p_residual_all, np.array([la_min*1.1, -chi_ini, -chi_ini]))
        roots = sol_object.x

        if np.any(np.abs(roots[1:]) < 1e-3) : # (chi_up, chi_dn) < 1e-3
            continue
        else:
            break
