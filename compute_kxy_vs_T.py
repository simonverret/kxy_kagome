...
def compute_kxy_all(B, D, J, T, la_min, chi_up_ini, chi_dn_ini):
    ...
    return kxy, la, chi_up, chi_dn


T_a = np.arange(0.5, 2.0, 0.1)[::-1]
kxy_a = np.empty(T_a.shape)

for i, t in enumerate(T_a):
    print("la_min = ", la_min)
    print("T = ", t)
    print("chi_ini = ", chi_up_ini)
    kxy, la, chi_up, chi_dn = compute_kxy_all(B, D, J, t, la_min, chi_up_ini, chi_dn_ini)
    (la_min, chi_up_ini, chi_dn_ini) = (la, chi_up, chi_dn)
    kxy_a[i] = kxy
